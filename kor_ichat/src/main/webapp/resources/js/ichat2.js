...
function goLast() {
	...
	$link = $('.chat_cont .bot_wrap:last').find('a:visible,button:visible,input:visible')
	if ($link.length > 0) {
		$link.eq(0).focus()
	}
}
...
function saveSearchLog(query){ //검색 통계 로그
	var isPlatform="M";
	var logEnterType = "S";
	if(!isMobile()){
		isPlatform="W";
	...
	let logKeyword = getSearchQuery(searchKeyword, "log");
...
function goSearch(query){ //스포츠 지원 포털 검색 이동, 로그 저장
	if(isHelp=="Y"){
		if(getMorphWord(query) != ""){
			searchKeyword = getMorphWord(searchKeyword);
		}
	}
	
	let searchQuery = getSearchQuery(searchKeyword, "search");
	
	goLink('https://portal.sports.or.kr/search/result.do?searchQuery='+searchQuery);
...
function goLoginPage(userSessionChk){ //로그인 페이지로 이동되는 confirm창
	...
	openConfirmPopup("로그인", str, function(){
		goLink('https://g1.sports.or.kr/member/login.do:mURL:https://g1.sports.or.kr/m/member/login.do');
		window.close();
	})
}
