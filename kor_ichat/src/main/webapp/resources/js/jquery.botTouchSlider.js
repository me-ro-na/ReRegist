 ...
 * @example
	...
	$('#target').botTouchSlider();
	$('.multi-target').botTouchSlider();
...
(function(factory) {
	...
	$.fn.botTouchSlider = function(settings) {
		
		$.fn.botTouchSlider.defaults = {
			mode: 'swipe',
			useMouse: true,
			roll: true,
			flexible: true,
		...
		var opts = $.extend(true, {}, $.fn.botTouchSlider.defaults, settings);
	...
	var botTouchSlider = {
		...
		init: function() {
			...
			if(this.opts.paging) {
				...
				for(var i=1; i<=len; ++i) {
					if(len >1){
						...
						paging += '<button type="button" class="ts-paging-btn" style="display:none">page' + i + '</button>';
					}
				}
			...
			//	this._tg.after(this._controls);
				$(this).append('<div class="button li"><button type="button" class="ts-prev">Prev</button><button type="button" class="ts-next">Next</button></div>');
...
*/
		...
		setSize: function(page) {
			...
			this._width = window.outerWidth-this.opts.pleft;
			...
			this._item_w = (this._width -(this.opts.margin *2) - (this._view - 1) * this.opts.gap) / this._view;
			...
			if(window.outerWidth > 1100){
				$(".menu_list_bot > a").css(transStyle);
				/*
				$(".menu_list").css("width","570");
				$(".ts-controls").css("display","none");*/
		...
		move: function(obj) {
			...
			if(this.opts.mode === 'swipe') {
				if(isTransition) {
					...
					if(obj.speed === undefined) {
						obj.tg.stop().css({
							'left': obj.to+this.opts.margin + 'px',
							'opacity': '1'
						});
					} else {
						obj.tg.stop().animate({
