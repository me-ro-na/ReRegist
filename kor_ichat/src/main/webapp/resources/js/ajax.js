    ...
    $.ajax({
        ...
        success: function (result) {	
        	...
        	if(result.success){ //true
				if (document.querySelector('#userId') && result.outdata) {
					document.querySelector('#userId').value = result.outdata.userId;
				}
        		if(callback)
        			callback(result.outdata, result.indata);
        	}else{
        		$("#loading").remove();
    	...
    	error:function(result){
			...
			if (result.status == 401) {
				openAlertPopup('연결 실패', "접속 시간이 길어져서 연결이 끊겼습니다.\n챗봇이 종료됩니다.")
			}
//    		location.reload();
 	    }
    });
