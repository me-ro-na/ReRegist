((config) => {
	...
	config.BUTTON_CLASS_OF_STATUS = {
		"1": {desc:"등록가능", title: "재등록", data: {action: "continue", reRegGb: "R"}},
...
//	현재 API 코드값
	let getCurrentProcessCode = () => {	return thisProcessCode; }
//	인입 API 코드값
	let getInitProcessCode = () => { return initCode; }

//	save용(이어하기)
	let getCurrentProcessSeqForSave = (seq) => {
...
//	API CODE 검증
	...
	return {
		getCurrentProcessCode: getCurrentProcessCode,
		getInitProcessCode: getInitProcessCode,
		setProcessCode: setProcessCode,
		getCurrentProcessSeqForSave: getCurrentProcessSeqForSave,
		apiCodeVerification: apiCodeVerification,
		setCodes: setCodes,
		getCodes: getCodes,
		setLogSeq: setLogSeq,
		getLogSeq: getLogSeq,
...
//	목록 조회
	regist.setAnswer = function(seqParam) {
		try {
			codes.setProcessCode(seqParam.code);
			codes.setInfo(seqParam);
			console.log(codes.getInfo());
			let $dfd = $.Deferred();

			continueService.process(seqParam).done(function(isUnder14) {
				if(isUnder14){
					$dfd.resolve({isUnder14:isUnder14});
				}else if(seqParam.code === 'R00'){
					$dfd.resolve();
				}else{
					let registerGbCode = reRegistConfig.REGISTER_GB_CODE[codes.getCurrentProcessCode()];
					let param = Regist.setApiCallParam({apiType: codes.getCurrentProcessCode(), gubun: registerGbCode});
					Regist.apiCall(param).done(function (data){
						$dfd.resolve(reRegistListView.process(data));
					});
				}
			});
...
//		외부 링크 열기
	...
	regist.changeHandler = function(event) {
		...
		switch (data.action){
			case 'agree':
				provisionAgreement.process(data);
				break;
			case 'law':
				prerequisiteAcademicPolicies.process(data);
				break;
			case 'detail':
				userInfo.process(data);
				break;
			case 'continue':
				continueService.process(data);
				break;
			case 'link':
				if(data.reRegGb !== undefined){
					codes.setInfo(data);
				}
				_sportsApi.moveSite('PINFO', codes.getInfo().reRegGb + codes.getInfo().gubun, codes.getInfo().pclassCd);
				break;
			case 'cancel':
				Regist.deleteStep();
				break;
	...
	regist.createAgreementPopup = function(options) {
		...
		function setPopupContents() {
			...
			$btnDiv.append(btns.draw(settings.buttons));
	...
	regist.updateStep = (param) => {
		let defaultParam = {
			apiLogSeq : codes.getLogSeq(),
			apiStep : codes.getCurrentProcessSeqForSave()
		}
	...
	regist.getCloseBtn = (action) => {
		return btns.draw({text:'<span class="ico_comm ico_pop_close">팝업닫기</span>', class:"btn_pop_close", data:{action:action}, onClick:Regist.changeHandler })
	}
})(window.Regist = window.Regist || {});

// 인입 답변 작성
const answers = (function() {
	let drawAnswer = (seqParams, msg) => {
		codes.setProcessCode(seqParams.code);
		let $main;
		if (Object.keys(reRegistConfig.INIT_BTNS).indexOf(seqParams.code) >= 0) {
			$main = reRegistListView.process(msg);
		} else {
			//check
		}
		return $main;
...
//	리스트 작성
	let drawList = (data) => {
		...
		if(listData){
			//set header
			let $table = $template.find('table');
			let $thead = $template.find('thead');
			$thead.before($('<colgroup>'))
				  .append($('<tr>'));

			theadArr.forEach(function (el){
				$table.find('colgroup').append($('<col>', {style:`width:${el.width}`}));
			...
			$.makeArray(listData).forEach(function (el){
				...
				let aIdx = 0;
				let btnOpt = "";
				console.log(codes.getInfo().code);
				if(reRegistConfig.BUTTON_CLASS_OF_STATUS[el['RE_REG_GB']] !== undefined){
					btnOpt = btns.draw({
						...
						class: "btn_info",
						data: $.extend({}, reRegistConfig.BUTTON_CLASS_OF_STATUS[el['RE_REG_GB']].data, {code:codes.getInfo().code, val:el, pclassNm:el['PCLASS_NM'], pclassCd: el['PCLASS_CD'], personNo: el['PERSON_NO'], regYear: el['REG_YEAR'], gubun:reRegistConfig.REGISTER_GB_CODE[codes.getCurrentProcessCode()]}),
		...
		setTimeout(function(){
			selectRegistData().done(function() {
				if(isUnder14()){
					...
					if (data.reRegGb !== undefined && data.reRegGb !== null && data.reRegGb.length > 0) {
						Regist.changeHandler({data: $.extend({}, data, {action: 'agree'})});
					}
					$dfd.resolve();
				}
			});
...
//	WISE_LOG_SPORTS_API테이블에 이어하기 데이터 select(공통화)
	...
	let isUnder14 = () => {
		if(typeof info.continueData === "boolean"){
			return info.continueData;
		}else{
			return false;
		}
	}
	...
	let continuePopup = (data) => {
		...
		if(Object.keys(continuesData).length !== 0) {
			...
			let btnOptions = [{
				text: "아니오",
				class: "pop_btn gray",
				data: $.extend(true, {}, data, {action: "cancel"}),
				onClick: Regist.changeHandler
			}, {
				text: "불러오기",
				class: "pop_btn blue",
				//data: $.extend(true, {}, {action: "detail", continueSeq: continueSeq, registerGb: registerGbCode}),
				data: $.extend(true, {}, data, apiParam, {action: "detail", apiStep:continuesData.apiStep}),
				onClick: continueService.loadUserInfo
			}];
			$confirm.find(".continue-btn").append(btns.draw(btnOptions));
...
//	api call
	let process = (data) => {
		...
		data.apiType = data.apiType || 'R05';
		codes.setInfo(data);

		Regist.apiCall(codes.getInfo(["pclassCd", "personNo", "apiType"]))
			.done(function(result){
				...
				if(isAgree(rData)){
					...
					drawAgreement($.extend({}, rData, data));
				}
			});
...
//	약관 출력 및 기존 동의내역 출력
	...
	let saveAgreement = (e) => {
		...
		if(isCheckboxAgreed()) {
			...
			codes.setInfo(param);
			let result = Regist.apiCall(codes.getInfo(["apiType", "termsVersion", "pclassCd", "agreeYn"]));
...
const prerequisiteAcademicPolicies = (function() {
	...
	let drawLaw = (data) => {
		...
		if(step == "1"){
				...
				contentText = `
					...
					<table class="privacy_cont people">
						<tbody>
							<tr>
								<td style="border:1px solid #A1A1A1; padding:10px;">
									<p>② 다음 각 호의 하나에 해당되는 사람은 지도자·심판·선수관리담당자로 등록할 수 없다.</p>
									<p>1. 피성년후견인</p>
									<p>2. 금고 이상의 실형을 선고받고 그 집행이 종료되거나 집행을 받지 아니하기로 확정된 후 5년이 경과하지 아니한 사람</p>
									<p>3. 금고 이상의 형을 선고받고 그 집행유예 기간이 끝난 날부터 2년이 지나지 아니한 사람</p>
									<p>4. 금고 이상의 형의 선고유예를 받은 경우에 그 선고유예 기간 중에 있는 사람</p>
									<p>5. 법원의 판결 또는 다른 법률에 따라 자격이 상실되거나 정지된 사람</p>
									<p>6. 체육회와 체육회 관계단체에서 재직기간 중 직무와 관련하여「형법」제355조 및 제356조에 규정된 죄를 범한 사람으로서 300만 원 이상의 벌금형을 선고받고 그 형이 확정된 후 2년이 지나지 아니한 사람</p>
									<p>7.「성폭력범죄의 처벌 등에 관한 특례법」 제2조에 규정된 죄를 범한 사람으로서 100만원 이상의 벌금형을 선고받고 그 형이 확정된 후 3년이 지나지 아니한 사람</p>
									<p>8. 미성년자에 대한 다음 각 목의 어느 하나에 해당하는 죄를 저질러 파면ㆍ해임되거나 형 또는 치료감호를 선고받아 그 형 또는 치료감호가 확정된 사람(집행유예를 선고받은 후 그 집행유예기간이 경과한 사람을 포함한다)</p>
									<p>&nbsp;가. 「성폭력범죄의 처벌 등에 관한 특례법」 제2조에 따른 성폭력범죄</p>
									<p>&nbsp;나. 「아동ㆍ청소년의 성보호에 관한 법률」 제2조제2호에 따른 아동ㆍ청소년대상 성범죄</p>
									<p>9. 체육회 관계단체가 주최ㆍ주관하는 경기의 결과에 영향을 미치는 승부조작에 가담하여「형법」 제314조 및 「국민체육진흥법」 제47조 및 제48조에 규정된 죄를 범한 사람으로서 벌금형 이상을 선고받고 그 형이 확정된 사람</p>
									<p>10. 다음 각 목의 어느 하나에 해당하는 죄를 저지른 사람으로서 금고 이상의 형 또는 치료감호를 선고받고 그 집행이 종료되거나 집행이 유예ㆍ면제된 날부터 20년이 지나지 아니하거나 벌금형이 확정된 날부터 10년이 지나지 아니한 사람</p>
									<p>&nbsp;가. 「성폭력범죄의 처벌 등에 관한 특례법」 제2조에 따른 성폭력범죄</p>
									<p>&nbsp;나. 「아동ㆍ청소년의 성보호에 관한 법률」 제2조제2호에 따른 아동ㆍ청소년대상 성범죄</p>
									<p>11. 선수를 대상으로 「형법」 제2편제25장 상해와 폭행의 죄를 저지른 자로서 금고 이상의 형을 선고받고 그 집행이 종료되거나 집행이 유예ㆍ면제된 날부터 10년이 지나지 아니한 사람</p>
									<p>12. 체육회 관계단체로부터 자격정지 이상의 징계처분을 받고 그 처분이 종료되지 아니한 사람</p>
									<p>13. 체육회 관계단체로부터 다음 각 목의 어느 하나에 해당하는 사유로 자격정지 1년 이상의 징계처분을 받은 사람</p>
									<p>&nbsp;가. 폭력‧성폭력</p>
									<p>&nbsp;나. 승부조작</p>
									<p>&nbsp;다. 편파판정</p>
									<p>&nbsp;라. 횡령‧배임</p>
									<p>14. 체육회 관계단체로부터 해임 징계를 받고 10년이 지나지 아니한 사람</p>
								</td>
							</tr>
						</tbody>
					</table>
			...
			action = $.extend(data, {action: 'law', step: 2});
		}else if(step == "2"){
			title = "경기인의 교육 의무";
			contentText = `<strong class="cont_tit_info" style="line-height: 1.5">대상: 경기인(회원종목단체에 등록한 선수(전문/생활), 지도자, 심판, 선수관리담당자)</strong>
					...
					`;
			action = $.extend(data, {action: 'detail'}); //comment: userInfo에서 필요한 data 추가,  data에서 꺼내면 됨 예) data.pclassCd
		}
...
const userInfo = (function() {
	...
	let process = (data) => {
		if(data){
			let apiStep = "0";
			if(data.apiStep && data.apiStep.length != ""){
				apiStep = data.apiStep;
			...
			Regist.apiCall(data)
				.done(function(result){
					if(!$.isEmptyObject(result)){
						setDataInfo($.extend({}, result.DATA, data));
						codes.setLogSeq(result.apiLogId);
						if(result.DATA !== ""){
	...
	let drawButton = ($template) => {
		...
		$template.filter(".user_info_btn").first().append(btns.draw([btn1, btn2]));
	}
